...
{
	TWSR = 0;
	TWBR = (uint8)((((F_CPU / frequency) / I2C_SCL_FREQUENCY_PRESCALER) - 16 ) / 2);
}
...
uint8 i2c_master_send(uint8 address, uint8* data, uint16 length)
{
	...
	uint8 status = i2c_master_start(address, I2C_WRITE);
...
{
	uint8 status = i2c_master_start(address, I2C_READ);
	if (status) return status;
