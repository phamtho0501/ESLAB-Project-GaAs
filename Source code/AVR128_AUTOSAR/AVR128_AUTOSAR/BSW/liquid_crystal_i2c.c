...
void lq_setCursor(LiquidCrystalDevice_t* device, uint8 row, uint8 column)
{
...
void lq_sendCommand(LiquidCrystalDevice_t* device, uint8 command)
{
	lq_writeDeviceByte(device, command, 0);
}

void lq_writeDeviceByte(LiquidCrystalDevice_t* device, uint8 value, uint8 mode)
{
...
void lq_writeDevice4Bits(LiquidCrystalDevice_t* device, uint8 value)
{
	lq_transmitI2C(device, value);
	lq_writeDevicePulse(device, value);
};

void lq_writeDevicePulse(LiquidCrystalDevice_t* device, uint8 value)
{
...
void lq_transmitI2C(LiquidCrystalDevice_t* device, uint8 value)
{
